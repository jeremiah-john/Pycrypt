...
def encrypt():
	...
	outputFile = raw_input("what is the name of the new file you want to write the encrypted message to?(file extensions also compatible(.txt,.torrent,etc)):");#now the user can specify the name of the file to write the encrypted message to
	keyFile = raw_input("create a name for the new key file (must end in .p):");
	passwordFile = raw_input("what is the name of the new file you want to write the hashed password to?:");
	...
	passwd = passwd;
	pickle.dump(key, open(keyFile, "wb+")); # where we will store the key. This is a very insecure way to store keys and needs to change
	encPass = hashlib.sha256(passwd).hexdigest(); #  moved to SHA256 
	passFile = open(passwordFile, "wb+");
...
def decrypt():
	...
	keyFile = raw_input("what is the name of the key file associated with your encrypted message?:");
	passwordFile = raw_input("what is the name of the password file associated with your encrypted message?:");
	message = open(inputFile, "rb");
	setPass = open(passwordFile, "rb");
	passStat = os.stat(passwordFile); # we need to find out the size of the password file so that we can efficiently print the encoded password to a variable to decode
	...
	cleanOutput = raw_input("what is the name of the file you want to write the decrypted message to? include file extension(windows users should use .txt for readability:");
